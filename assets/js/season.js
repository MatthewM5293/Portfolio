 ...
 * Automatically applies seasonal color themes based on the current date
 * 
 * Seasons:
 * - Fall: September 22 - December 20 (Orange)
 * - Winter: December 21 - March 19 (Holly Red)
 * - Spring: March 20 - June 20 (Green)
 * - Summer: June 21 - September 21 (Golden Yellow)
 */

(function() {
    'use strict';

    // Season configuration with SVG pattern colors
    const seasons = {
        winter: {
            className: 'season-winter',
            patternColor: 'rgba(45, 106, 159, 0.25)',
        ...
        spring: {
            ...
            patternColor: 'rgba(74, 158, 100, 0.25)',
            // March 20 to June 20
        ...
        summer: {
            ...
            patternColor: 'rgba(110, 83, 16, 0.25)',
        ...
        fall: {
            ...
            patternColor: 'rgba(111, 52, 0, 0.25)',
    ...
    function getSeason(date) {
        ...
        // Check each season
        if ((month === 12 && day >= 21) || (month <= 3 && (month < 3 || day <= 19))) {
            return 'winter';
    ...
    function applySeasonalTheme() {
        ...
        document.body.classList.add(seasonConfig.className);
        
        // Update the wrapper background pattern if it exists
        const wrapper = document.getElementById('wrapper');
        if (wrapper) {
            // Create a style element for the dynamic pattern
            let styleEl = document.getElementById('season-pattern-style');
            if (!styleEl) {
                styleEl = document.createElement('style');
                styleEl.id = 'season-pattern-style';
                document.head.appendChild(styleEl);
            }
            
            styleEl.textContent = `
                #wrapper:before {
                    background-image: ${generatePatternUrl(seasonConfig.patternColor)};
                }
                select {
                    background-image: ${generateSelectArrowUrl(seasonConfig.patternColor)};
                }
            `;
        }
        
        // Log the current season for debugging
        console.log(`🎨 Seasonal theme applied: ${currentSeason}`);
    }
